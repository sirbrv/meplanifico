...
import GrupoGastoModal from "./GrupoGastoModal";
import ListTable from "./ListTableGrupoGasto";
import InfoModal from "../../../componets/Informacion";
import VerifyDelete from "../../../componets/VerifyDelete";
import Pagination from "../../../componets/Pagination";
import "../../ab.css";

function ListarGrupoGasto() {
    ...
    axiosFetch({
      ...
      .then((response) => {
        if (response.status === "200") {
          setGrupoGasto(response.data.grupoGastos);
          ...
          if (response.msg === undefined) {
            setInformacionModal({
              isOpen: true,
              title: "Listado de Grupo de Gastos",
              body: response,
            });
          } else {
            setInformacionModal({
  ...
  const deleteGrupoGasto = async (reg) => {
  ...
  const getGrupoGasto = async () => {
    await getData();
  };

  //*************  Funciones  axios*********** */

  const handdleAddGrupoGasto = () => {
    setAddModal(true);
  };

  const handdleEditGrupoGasto = (id) => {
    setRegister(id);
    setEditModal(true);
  };
  ...
  const handleDelete = (id) => {
    ...
    setVerifyDeleteModal({
      isOpen: true,
      title: "Eliminación de Grupo de Gastos",
  ...
  return (
    <Container>
        ...
        <div className="mx-4 my-3">
          <Row className="m-3 d-flex align-items-center m -3">
            <Col className="" sm={9} xs={8}>
              <h3 className="fsize">Grupo de Gastos</h3>
            </Col>
      ...
      {login && !loading && !error && grupoGasto && (
        <>
          <Card className="mb-3 mx-3 shadow">
            <ListTable
              datas={grupoGasto}
              handdleEditGrupoGasto={handdleEditGrupoGasto}
      ...
      {addModa && (
        <GrupoGastoModal
          tittle="Adicionar Grupo de Gasto"
          registro=""
          onClose={() => {
            setAddModal(false);
          }}
      ...
      {editModa && (
        <GrupoGastoModal
          tittle="Actualizar Grupo de Gasto"
          registro={register}
          onClose={() => {
            setEditModal(false);
          }}
...
export default ListarGrupoGasto;
