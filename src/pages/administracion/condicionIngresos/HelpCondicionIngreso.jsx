...
import ListTables from "./ListTableCondicionIngreso";
import InfoModal from "../../../componets/Informacion";
import Pagination from "../../../componets/Pagination";
import "../../ab.css";

function HelpCondicionIngresos({ onClose, handleSelect }) {
  const [show, setShow] = useState(false);
  const [condicionIngresos, setCondicionIngresos] = useState([]);
    ...
    axiosFetch({
      ...
      .then((response) => {
        if (response.status === "200") {
          ...
          setCondicionIngresos(response.data.Condiciones);
          ...
          if (response.msg === undefined) {
            setInformacionModal({
              isOpen: true,
              title: "Listado de Condiciónes de Ingresos",
              body: response,
            });
          } else {
            setInformacionModal({
  ...
  const getCondicionIngresos = async () => {
    await getData();
  };
  ...
  const tittle = "Ayuda de Condiciones de Ingresos";
    ...
    <>
      <Modal show={show} onHide={handleClose} size="md">
        <div className="m-1 shadow fsize">
          ...
          {!loading && !error && condicionIngresos && (
            <div>
              <Card className="mb-3 mx-3 shadow">
                <ListTables
                  datas={condicionIngresos}
...
export default HelpCondicionIngresos;
