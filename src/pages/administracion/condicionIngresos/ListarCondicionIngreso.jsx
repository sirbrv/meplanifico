...
import CondicionIngresoModal from "./CondicionIngresoModal";
import ListTable from "./ListTableCondicionIngreso";
...
function ListarCondicionIngreso() {
    ...
    axiosFetch({
      ...
      .then((response) => {
        if (response.status === "200") {
          setCondicionIngreso(response.data.condiciones);
          ...
          if (response.msg === undefined) {
            setInformacionModal({
              isOpen: true,
              title: "Listado de Condiciones de Ingreso",
              body: response,
            });
          } else {
            setInformacionModal({
  ...
  const deleteCondicionIngreso = async (reg) => {
  ...
  const getCondicionIngreso = async () => {
    await getData();
  };

  //*************  Funciones  axios*********** */

  const handdleAddCondicionIngreso = () => {
    setAddModal(true);
  };

  const handdleEditCondicionIngreso = (id) => {
    setRegister(id);
    setEditModal(true);
  };
  ...
  const handleDelete = (id) => {
    ...
    setVerifyDeleteModal({
      isOpen: true,
      title: "Eliminación de Condiciones de Ingreso",
  ...
  return (
    <Container>
      ...
      {!loading && !error && condicionIngreso && (
        <>
          <Card className="mb-3 mx-3 shadow">
            <ListTable
              datas={condicionIngreso}
              handdleEditCondicionIngreso={handdleEditCondicionIngreso}
      ...
      {addModa && (
        <CondicionIngresoModal
          IngresoModal
          tittle="Adicionar Condicion de Ingreso"
          registro=""
          onClose={() => {
            setAddModal(false);
          }}
      ...
      {editModa && (
        <CondicionIngresoModal
          tittle="Actualizar Condición de Ingreso"
          registro={register}
          onClose={() => {
            setEditModal(false);
          }}
      ...
      {verifyDeleteModal.isOpen && (
        <VerifyDelete
          ...
          onConfirmar={() => {
            deleteCondicionIngreso(register);
            setVerifyDeleteModal({ isOpen: false, title: "", body: "" });
          }}
        />
      )}
...
export default ListarCondicionIngreso;
