...
import TipoIngresoModal from "./TipoIngresoModal";
import ListTable from "./ListTableTipoIngreso";
import InfoModal from "../../../componets/Informacion";
import VerifyDelete from "../../../componets/VerifyDelete";
import Pagination from "../../../componets/Pagination";
import "../../ab.css";

function ListarTipoIngreso() {
    ...
    axiosFetch({
      ...
      .then((response) => {
        if (response.status === "200" || "201") {
          setTipoIngreso(response.data.tipoIngresos);
          ...
          if (response.msg === undefined) {
            setInformacionModal({
              isOpen: true,
              title: "Listado de tipo de Ingresos",
              body: response,
            });
          } else {
            setInformacionModal({
  ...
  const deleteTipoIngreso = async (reg) => {
  ...
  const handdleAddTipoIngreso = () => {
    setAddModal(true);
  };

  const handdleEditTipoIngreso = (id) => {
    setRegister(id);
    setEditModal(true);
  };
  ...
  const handleDelete = (id) => {
    ...
    setVerifyDeleteModal({
      isOpen: true,
      title: "Eliminación de Tipo de Ingresos",
  ...
  return (
    <Container>
      ...
      {login && !loading && !error && tipoIngreso && (
        <>
          <Card className="mb-3 mx-3 shadow">
            <ListTable
              datas={tipoIngreso}
              handdleEditTipoIngreso={handdleEditTipoIngreso}
      ...
      {addModa && (
        <TipoIngresoModal
          tittle="Adicionar Tipo de Ingreso"
          registro=""
          onClose={() => {
            setAddModal(false);
          }}
      ...
      {editModa && (
        <TipoIngresoModal
          tittle="Actualizar Tipo de Ingreso"
          registro={register}
          onClose={() => {
            setEditModal(false);
          }}
...
export default ListarTipoIngreso;
