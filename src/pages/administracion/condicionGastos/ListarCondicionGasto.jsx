...
function ListarCondicionGasto() {
    ...
    axiosFetch({
      ...
      .then((response) => {
        if (response.status === "200") {
          setCondicionGasto(response.data.condiciones);
          ...
          if (response.msg === undefined) {
            setInformacionModal({
              isOpen: true,
              title: "Listado de Condiciones de Gasto",
              body: response,
            });
          } else {
            setInformacionModal({
  ...
  const deleteCondicionGasto = async (reg) => {
  ...
  const getCondicionGasto = async () => {
    await getData();
  };

  //*************  Funciones  axios*********** */

  const handdleAddCondicionGasto = () => {
    setAddModal(true);
  };

  const handdleEditCondicionGasto = (id) => {
    setRegister(id);
    setEditModal(true);
  };
  ...
  const handleDelete = (id) => {
    ...
    setVerifyDeleteModal({
      isOpen: true,
      title: "Eliminación de Condiciones de Gasto",
  ...
  return (
    <Container>
      ...
      {login && !loading && !error && condicionGasto && (
        <>
          <Card className="mb-3 mx-3 shadow">
            <ListTable
              datas={condicionGasto}
              handdleEditCondicionGasto={handdleEditCondicionGasto}
      ...
      {addModa && (
        <CondicionGastoModal
          GastoModal
          tittle="Adicionar Condicion de Gasto"
          registro=""
          onClose={() => {
            setAddModal(false);
          }}
      ...
      {editModa && (
        <CondicionGastoModal
          tittle="Actualizar Condición de Gasto"
          registro={register}
          onClose={() => {
            setEditModal(false);
          }}
...
export default ListarCondicionGasto;
